...
export class Auth {
  ...
  reset(...args: unknown[]): void {
    ...
    return this.getStrategy().reset(...(args as [options?: { resetInterceptor: boolean }]))
  }
  ...
  refreshTokens(): Promise<HTTPResponse | void> {
    ...
    return Promise.resolve((this.getStrategy() as RefreshableScheme).refreshController.handleRefresh()).catch(
      (error) => {
  ...
  requestWith(strategy: string, endpoint: HTTPRequest, defaults?: HTTPRequest): Promise<HTTPResponse> {
    ...
    const tokenName = (this.strategies[strategy] as TokenableScheme).options.token.name || "Authorization"
