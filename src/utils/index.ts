...
export function isSet(o: unknown): boolean {
  return !isUnset(o)
}

export function isSameURL(ctx: Nuxt, a: string, b: string): boolean {
  return normalizePath(a, ctx) === normalizePath(b, ctx)
}
...
export function isRelativeURL(u: string): boolean {
  return (
    u !== "" &&
    u.length > 0 &&
    new RegExp(["^\\/([a-zA-Z0-9@\\-%_~.:]", "[/a-zA-Z0-9@\\-%_~.:]*)?", "([?][^#]*)?(#[^#]*)?$"].join("")).test(u)
  )
}
...
export function routeOption(route: Route, key: string, value: string | boolean): boolean {
...
export function getMatchedComponents(route: Route, matches: unknown[] = []): unknown[] {
  return [].concat(
    ...[],
...
export function normalizePath(path = "", ctx?: Nuxt): string {
  // Remove query string
  let result = path.split("?")[0]

  // Remove base path
  ctx?.options.runtimeConfig
  if (ctx && ctx.base) {
