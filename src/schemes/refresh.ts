...
export class RefreshScheme<OptionsT extends RefreshSchemeOptions = RefreshSchemeOptions>
  ...
  implements RefreshableScheme<OptionsT>
{
  ...
  protected updateTokens(response: HTTPResponse, { isRefreshing = false, updateOnRefresh = true } = {}): void {
    const token = this.options.token.required ? (getProp(response.data, this.options.token.property) as string) : true
